...
面向普通用户的男生 AI 发型分析 H5。上传一张正面肖像后，应用会提供：

- 最佳短发、最佳中发、最佳长发和一款谨慎选择建议
- 用户从三款推荐中选择一款生成真人预览，整单最多调用 Image2 两次
- 两款辅助发色色卡
...
生产环境只允许 Kie，并固定为 Terra 分析、Luna 常规质检、Terra 疑难复核与 Image2 图生图。任何其他模型配置都会失败关闭。API Key 只允许保存在本地未提交的环境文件或托管平台加密环境变量中。
...
- Kie 积分低于默认 120 时暂停新任务；连续服务故障会短时熔断。
