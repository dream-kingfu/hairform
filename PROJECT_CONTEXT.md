...
- 开启真人预览后，用户可从最佳短发、中发、长发和两款推荐发色中先选一款，再点击最后确认生成；Image2 每任务最多调用两次，第二次只用于质检失败后的重试。
- 仅“谨慎选择”不允许触发 Image2；选中动作本身不调用模型，最终确认才调用。
- 真人预览失败不会影响已经完成的文字报告。
...
- 根据脸型、发质、发量、发际线和肤色倾向生成的确定性自然语言建议。
- 两款可选中生成真人效果的推荐发色色卡。
- 三款推荐发型对应的理发师沟通卡。
- `2160×3840 PNG` 高清报告。
...
```text
analysis_ready
  → 用户选择 best_short / best_medium / best_long / color_primary / color_secondary
  → 页面显示最终确认，不在选择时直接生成
  → 用户确认生成
  → generating
...
- 真人预览开启后，发型与发色卡先显示“选这款”，选中后再出现固定的最终生成按钮。
- 修复了 `analysis_ready` 状态下网页按钮显示但点击被旧状态判断拦截的问题。
- 生成成功后自动重排报告，完整包含原图、所选真人效果、自然语言建议、发型建议、发色色卡和整体风格。
...
- 发型和发色均支持“选这款 → 最后确认 → 生成完整预览”，按钮由服务器开关控制。
...
- 生产构建通过。
- Lint 通过。
- 22 项 H5/后端自动化测试与 5 项微信小程序自动化测试通过。
...
- 网页与微信小程序新增“先选后生成”两步交互，发型和发色选择本身都不会立即调用 Image2。
- 两款推荐发色进入单图真人预览链路；生成完成后自动重排完整高清报告。
- 推荐卡增加根据视觉分析结果生成的自然语言说明，让建议更像发型顾问。
- 新增与产品视觉一致的社交分享封面与 Open Graph / X 元数据。

#### Fixed

- 修复 H5 在 `analysis_ready` 状态下生成按钮点击无效的问题。
- 发型与发色统一使用同一套选择锁、异步生成、质检、失败重试与报告流程。

#### Verified

- Lint、生产构建、22 项 H5/后端自动化测试、5 项微信小程序自动化测试和小程序脚本语法检查通过。
...
- 发型与发色卡先选中、再通过固定确认栏生成，避免误触调用。
- 两款推荐发色可生成真人效果，完整报告会根据所选发型或发色重排。
- 推荐卡增加更自然的个性化说明。
- 5 项自动化测试与脚本语法检查通过；尚需微信开发者工具和真机验收。
